...
| **L4 + LLM 第三层 = 0.9551，追平 L1**（规则臂 0.8486）——REPORT 旧句 "0.8330 → 0.9327" 应刷新为 "0.8486 → 0.9551"。⚠️ **该行须带限定**：实验 35 在 GLM 免费额度上复现失败（429 限流触发静默熔断），实验 36 换 DeepSeek 付费端点复核**数字成立**（差异在噪声带内）——引用时请加 "measured on a paid, rate-limit-free endpoint"。另：该行 L3(0.9640)>L2(0.9547) 单调性倒置可复现、机理未定位，建议加脚注 "observed, mechanism not yet isolated" | 实验 33（A）/ 35→36（C） |
